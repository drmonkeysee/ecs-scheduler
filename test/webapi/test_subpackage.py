...
class CreateTests(unittest.TestCase):
    ...
    def test_create_server(self, fake_flask, fake_flask_restful, fake_cors):
        fake_flask.return_value.config = {}
        ...
        fake_flask_restful.return_value.add_resource.assert_any_call(ecs_scheduler.webapi.jobs.Jobs, '/jobs', resource_class_args=(self._queue, self._dc))
        fake_flask_restful.return_value.add_resource.assert_any_call(ecs_scheduler.webapi.jobs.Job, '/jobs/<job_id>', resource_class_args=(self._queue, self._dc))
        ...
        self.assertFalse(fake_flask.return_value.config['ERROR_404_HELP'])

    @patch('logging.getLogger')
    def test_adds_file_handler_if_present(self, get_log, fake_flask, fake_flask_restful, fake_cors):
        fake_flask.return_value.config = {}
