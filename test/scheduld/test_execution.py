...
class JobExecutorTests(unittest.TestCase):
    ...
    def test_call_does_nothing_if_more_than_expected_task_count(
        ...
        fake_trigger.determine_task_count.return_value = 3
        fake_get_trigger.return_value = fake_trigger
        self._exec._ecs.list_tasks.return_value = {
            'taskArns': ['a', 'b', 'c', 'd', 'e'],
        }
    ...
    def test_call_launches_tasks_if_none_running(self, fake_get_trigger):
        ...
        fake_get_trigger.return_value = fake_trigger
        self._exec._ecs.list_tasks.return_value = {'taskArns': []}
        self._exec._ecs.run_task.return_value = {
            'tasks': [
                {'taskArn': 'foo1', 'containerInstanceArn': 'bar1'},
                {'taskArn': 'foo2', 'containerInstanceArn': 'bar2'},
            ],
            'failures': [],
        }
        ...
        self._exec._ecs.run_task.assert_called_with(
            cluster='testCluster', taskDefinition='foo', count=3,
            startedBy='testName'
        )
        fake_get_trigger.assert_called_with(None)
    ...
    def test_call_uses_default_name_if_not_specified(self, fake_get_trigger):
        with patch('boto3.client'), \
            patch.dict(
                os.environ, {'ECSS_ECS_CLUSTER': 'testCluster'}, clear=True
        ):
        ...
        executor._ecs.run_task.return_value = {
            'tasks': [
                {'taskArn': 'foo1', 'containerInstanceArn': 'bar1'},
                {'taskArn': 'foo2', 'containerInstanceArn': 'bar2'},
            ],
    ...
    def test_call_logs_warning_if_all_tasks_fail(
        ...
        self._exec._ecs.run_task.return_value = {
            'tasks': [],
            'failures': [
                {'arn': 'failedTask', 'reason': 'poop'},
                {'arn': 'failedTask2', 'reason': 'poop'},
                {'arn': 'failedTask3', 'reason': 'poop'},
            ],
        }
    ...
    def test_call_launches_tasks_if_some_running(self, fake_get_trigger):
        ...
        self._exec._ecs.run_task.assert_called_with(
            cluster='testCluster', taskDefinition='foo', count=2,
            startedBy='testName'
        )
        fake_get_trigger.assert_called_with(None)
    ...
    def test_call_launches_max_single_batch_tasks(self, fake_get_trigger):
        ...
        self._exec._ecs.run_task.assert_called_with(
            cluster='testCluster', taskDefinition='foo', count=10,
            startedBy='testName'
        )
    ...
    def test_call_launches_multiple_task_batches(self, fake_get_trigger):
        ...
        fake_get_trigger.return_value = fake_trigger
        self._exec._ecs.list_tasks.return_value = {'taskArns': []}
        self._exec._ecs.run_task.side_effect = (
            {
                'tasks': [
                    {'taskArn': 'foo1', 'containerInstanceArn': 'bar1'},
                    {'taskArn': 'foo2', 'containerInstanceArn': 'bar2'},
            ...
            {
                'tasks': [
                    {'taskArn': 'faz1', 'containerInstanceArn': 'baz1'},
                    {'taskArn': 'faz2', 'containerInstanceArn': 'baz2'},
                ],
                'failures': [],
            },
        )
        ...
        self.assertEqual([
            {'taskId': 'foo1', 'hostId': 'bar1'},
            {'taskId': 'foo2', 'hostId': 'bar2'},
            {'taskId': 'faz1', 'hostId': 'baz1'},
            {'taskId': 'faz2', 'hostId': 'baz2'},
        ], result.task_info)
        self._exec._ecs.run_task.assert_any_call(
            cluster='testCluster',
            taskDefinition='foo',
            count=10,
            startedBy='testName'
    ...
    def test_call_launches_single_batch_if_running_count_reduces_expected_count_to_one_batch(self, fake_get_trigger):
        ...
        self.assertEqual([], result.task_info)
        self._exec._ecs.run_task.assert_called_with(
            cluster='testCluster', taskDefinition='foo', count=9,
            startedBy='testName'
        )
        self.assertEqual(1, self._exec._ecs.run_task.call_count)
        ...
        expected_overrides = [
            {
                ...
                'environment': [
                    {'name': 'foo', 'value': 'bar'},
                    {'name': 'baz', 'value': 'bort'},
                    {'name': self._exec.OVERRIDE_TAG, 'value': 'job-id'},
                ],
            },
        ]
        self.assertEqual(JobExecutor.RETVAL_STARTED_TASKS, result.return_code)
    ...
    def test_call_checks_override_tags_with_over_container_sections(
        ...
        self._exec._ecs.describe_tasks.return_value = {
            'tasks': [
                ...
                {'overrides': {'containerOverrides': [
                    {'name': 'b', 'environment': [
                        {'name': 'bort', 'value': 'blarg'},
                    ]},
                    {'name': 'b-2', 'environment': [
                        {'name': self._exec.OVERRIDE_TAG, 'value': 'job-id'},
                    ]},
                ]}},
                {'overrides': {'containerOverrides': [
                ...
                {'overrides': {'containerOverrides': [
                    ...
                    {'name': 'f-2', 'environment': [
                        {'name': self._exec.OVERRIDE_TAG, 'value': 'job-id'},
                    ]},
                    {'name': 'f-3', 'environment': [
                        {'name': 'baz', 'value': 'bort'},
                    ]},
                ]}},
                {'overrides': {'containerOverrides': [
    ...
    def test_call_does_not_match_other_id_in_override_tag(
        ...
        self._exec._ecs.describe_tasks.return_value = {
            'tasks': [
                ...
                {'overrides': {'containerOverrides': [
                    {'name': 'b', 'environment': [
                        {'name': self._exec.OVERRIDE_TAG, 'value': 'notme'},
                    ]},
                ]}},
                {'overrides': {'containerOverrides': [
                    {'name': 'c'},
                ...
                {'overrides': {'containerOverrides': [
                    {'name': 'e', 'environment': [
                        {'name': self._exec.OVERRIDE_TAG, 'value': 'boobar'},
                    ]},
                ]}},
                {'overrides': {'containerOverrides': [
                    {'name': 'f'},
    ...
    def test_call_uses_task_description_instead_of_id_if_present(
        ...
        self.assertEqual([], result.task_info)
        self._exec._ecs.run_task.assert_called_with(
            cluster='testCluster', taskDefinition='bar', count=3,
            startedBy='testName'
        )
        fake_get_trigger.assert_called_with(None)
    ...
    def test_call_uses_passes_overrides_to_ecs(self, fake_get_trigger):
        ...
        expected_overrides = [{
            ...
            'environment': [
                {'name': 'foo', 'value': 'bar'},
                {'name': self._exec.OVERRIDE_TAG, 'value': 'test-id'},
            ],
        }]
        self.assertEqual(JobExecutor.RETVAL_STARTED_TASKS, result.return_code)
        self.assertEqual([], result.task_info)
    ...
    def test_call_uses_passes_overrides_with_multiple_entries_to_ecs(
        ...
        expected_overrides = [
            {'name': 'test-container1', 'environment': [
                {'name': 'foo', 'value': 'bar'},
                {'name': self._exec.OVERRIDE_TAG, 'value': 'test-id'},
            ]},
            {'name': 'test-container2', 'environment': [
                {'name': 'boo', 'value': 'baz'},
                {'name': self._exec.OVERRIDE_TAG, 'value': 'test-id'},
            ]},
        ]
