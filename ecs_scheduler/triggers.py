...
def get(trigger_name):
    ...
    :returns: The trigger for the given name or the NoOpTrigger
              if no such trigger is found
    """
...
class SqsTrigger:
    ...
    def determine_task_count(self, job_data):
        ...
        :returns: The desired ECS task count based on the number of messages
                  in the queue and the desired scale factor in the job
                  definition; at a minimum will return the task count
                  of the job
        """
    ...
    def _calculate_task_count(self, message_count, job_data):
        ...
        scaled_task_count = (
            math.ceil(message_count / scaling_factor)
            if scaling_factor
            else 0
        )
