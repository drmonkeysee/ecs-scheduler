...
class TriggerSchema(marshmallow.Schema):
    ...
    Schedule triggers are used to schedule a job based on an AWS event instead
    of a date/time point.
...
class JobSchema(marshmallow.Schema):
    ...
    A job is an ecs scheduler document that defines a run schedule
    for an ECS task.
    """
    _WILD_CARD = '?'
